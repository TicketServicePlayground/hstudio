export const clients = [
  {
    logo: '/img/clients/logos_x2/logo-10.png',
    name: 'PlayDeck',
    bgImg: '/img/clients/gradients/10.png',
    mobileBgImg: '/img/clients/gradients/10.png',
    cardDesktop: '/img/clients/bg/PlayDeckCard.png',
    bgStyle: '',
    categories: ['game', 'web'],
    contentImg: '',
    tags: [
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
      {
        text: 'game',
        bgColor: 'lime',
        textColor: 'black',
      },
    ],
    bio: 'PlayDeck',
    href: 'https://t.me/playdeckbot/',
  },
  {
    logo: '/img/clients/logos_x2/logo-1.png',
    name: 'VTB — Data Streaming System',
    bgImg: '/img/clients/gradients/2.jpeg',
    cardDesktop: '/img/clients/gradients/2.jpeg',
    bgStyle: '',
    categories: ['backend', 'devops'],
    tags: [
      {
        text: 'backend',
        bgColor: 'liliac',
        textColor: 'black',
      },
      {
        text: 'devops',
        bgColor: 'blue',
        textColor: 'black',
      },
    ],
    bio: 'VTB',
    stats: [
      {
        label: 'Team Size',
        value: '5 people',
      },
      {
        label: 'Duration',
        value: '9 months',
      },
    ],
  },
  {
    logo: '/img/clients/logos_x2/logo-2.png',
    name: 'Sber — Microservice Architecture and Biometric Authorization',
    bgImg: '/img/clients/gradients/3.jpeg',
    categories: ['web', 'devops', 'seamless'],
    mobileBgImg: '/card-sber.png',
    cardDesktop: '/img/clients/gradients/3.jpeg',
    bgStyle: '',
    tags: [
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
      {
        text: 'devops',
        bgColor: 'blue',
        textColor: 'black',
      },
      {
        text: 'seamless',
        bgColor: 'cardDark',
        textColor: 'white',
      },
    ],
    bio: 'Sber',
    stats: [
      {
        label: 'Team Size',
        value: '5 people',
      },
      {
        label: 'Duration',
        value: '18 months',
      },
    ],
  },
  {
    logo: '/img/clients/logos_x2/logo-3.png',
    name: 'Societe Generale — Personalized Advertising Banner Service',
    bgImg: '/img/clients/bg/bg-4.png',
    bgStyle: '',
    cardDesktop: '/img/clients/bg/bg-4.png',
    categories: ['web', 'devops', 'seamless'],
    contentImg: '',
    tags: [
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
      {
        text: 'devops',
        bgColor: 'blue',
        textColor: 'black',
      },
      {
        text: 'seamless',
        bgColor: 'cardDark', // or darkCard
        textColor: 'white',
      },
    ],
    bio: 'Societe',
    stats: [
      {
        label: 'Team Size',
        value: '5 people',
      },
      {
        label: 'Duration',
        value: '12 months',
      },
    ],
  },
  {
    logo: '/img/clients/logos_x2/logo-4.png',
    name: 'Architectural bureau MIR',
    bgImg: '/img/clients/gradients/5.jpeg',
    bgStyle: '',
    cardDesktop: '/img/clients/bg/MirBackgroundCard.png',
    contentImg: '',
    categories: ['design', 'devops'],
    tags: [
      {
        text: 'design',
        bgColor: '[#ECEBF1]',
        textColor: 'black',
      },
    ],
    bio: 'Architectural',
    stats: [
      {
        label: 'Team Size',
        value: '3 people',
      },
      {
        label: 'Duration',
        value: '6 months',
      },
    ],
    href: 'https://abs-mir.com/eng',
  },
  {
    logo: '/img/clients/logos_x2/logo-0.png',
    name: 'EventStripe',
    bgImg: '/img/clients/gradients/1.jpeg',
    mobileBgImg: '/card-eventstripe-mobile.png',
    bgStyle: '',
    cardDesktop: '/img/clients/bg/EventStripeCardInside.png',
    bgStyleMobile: '',
    categories: ['web', 'devops'],
    contentImg: '',
    tags: [
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
      {
        text: 'devops',
        bgColor: 'blue',
        textColor: 'black',
      },
    ],
    bio: 'EventStripe',
    stats: [
      {
        label: 'Team Size',
        value: '3 people',
      },
      {
        label: 'Duration',
        value: '12 months',
      },
    ],
  },
  {
    logo: '/img/clients/logos_x2/logo-5.png',
    name: 'KClub Group',
    bgImg: '/img/clients/gradients/6.jpeg',
    mobileBgImg: '/card-kclub.png',
    cardDesktop: '/img/clients/bg/KClubCard.png',
    bgStyle: '',
    categories: ['backend', 'web'],
    contentImg: '',
    tags: [
      {
        text: 'backend',
        bgColor: 'liliac',
        textColor: 'black',
      },
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
    ],
    bio: 'KClub',
    href: 'https://www.kclubgroup.com/',
  },
  {
    logo: '/img/clients/logos_x2/logo-6.png',
    name: 'Matahari Hills',
    cardDesktop: '/img/clients/bg/HatariCard.png',
    bgImg: '/img/clients/bg/bg-7.png',
    bgStyle: '',
    contentImg: '',
    categories: ['backend', 'web', 'seamless'],
    tags: [
      {
        text: 'backend',
        bgColor: 'liliac',
        textColor: 'black',
      },
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
      {
        text: 'seamless',
        bgColor: 'cardDark', // or darkCard
        textColor: 'white',
      },
    ],
    bio: 'Matahari',
    href: 'https://www.mataharihills.com',
  },
  {
    logo: '/img/clients/logos_x2/logo-7.png',
    name: 'Mirage Flash',
    bgImg: '/img/clients/gradients/8.jpeg',
    mobileBgImg: '/card-mirage-mobile.png',
    bgStyle: '',
    cardDesktop: '/img/clients/bg/MirageFlashCard.png',
    categories: ['backend', 'web', 'design'],
    contentImg: '',
    tags: [
      {
        text: 'backend',
        bgColor: 'liliac',
        textColor: 'black',
      },
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
      {
        text: 'design',
        bgColor: '[#ECEBF1]',
        textColor: 'black',
      },
    ],
    bio: 'Mirage',
    href: 'https://www.mirageflash.com',
  },
  {
    logo: '/img/clients/logos_x2/logo-8.png',
    name: 'Frigate Surfboards',
    bgImg: '/img/clients/gradients/9.jpeg',
    mobileBgImg: '/card-frigate.png',
    bgStyle: '',
    cardDesktop: '/img/clients/bg/SurfBoardCard.png',
    contentImg: '',
    categories: ['backend', 'web', 'devops', 'seamless'],

    tags: [
      {
        text: 'backend',
        bgColor: 'liliac',
        textColor: 'black',
      },
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
      {
        text: 'devops',
        bgColor: 'blue',
        textColor: 'black',
      },
      {
        text: 'seamless',
        bgColor: 'cardDark', // or darkCard
        textColor: 'white',
      },
    ],
    bio: 'Frigate',
    href: 'https://www.frigate-surfboards.com/',
  },
  {
    logo: '/img/clients/logos_x2/logo-9.png',
    name: 'Ntainers',
    bgImg: '/img/clients/bg/bg-10.png',
    bgStyle: '',
    cardDesktop: '/img/clients/bg/NtainersCard.png',
    categories: ['backend', 'web', 'seamless'],

    contentImg: '',
    tags: [
      {
        text: 'backend',
        bgColor: 'liliac',
        textColor: 'black',
      },
      {
        text: 'web',
        bgColor: 'orange',
        textColor: 'black',
      },
      {
        text: 'seamless',
        bgColor: 'cardDark', // or darkCard
        textColor: 'white',
      },
    ],
    bio: 'Ntainers',
    href: 'https://ntainers.co',
  },
]

export type ClientT = typeof clients[0]


// DESKTOP:
//

// 1: background: url(<path-to-image>) lightgray 1613271183.534px 21642373537.46px / -2630893214.942% -3946339269.127% no-repeat, #FFF;
// background: url(<path-to-image>) lightgray 160141537.993px 11507298215.929px / -3424220217.623% -3538360834.15% no-repeat, #FFF;

// 2: background: url(<path-to-image>) lightgray 50% / cover no-repeat;
// background: url(<path-to-image>) lightgray 50% / cover no-repeat, #FFF;

// 3: background: url(<path-to-image>) lightgray 800px 696.253px / -124.048% -186.072% no-repeat, #FFF;
// background: url(<path-to-image>) lightgray 371.534px 469.603px / -139.7% -144.356% no-repeat;

// 4: top: 0, left: 31, right: -81,
//   top: 18.09, left: -21, right: -65

// 5: background: url(<path-to-image>) lightgray 50% / cover no-repeat, #FFF;
// background: url(<path-to-image>) lightgray 50% / cover no-repeat, #FFF;

// 6: background: url(<path-to-image>) lightgray 3751883249.45px 27177949183.563px / -2839480792.969% -4259220544.957% no-repeat;
// background: url(<path-to-image>) lightgray -387190631048.858px -61638225101.191px / 985482238269.935% 14870869251.746% no-repeat, #FFF;

// 7: top: -16, left: -36, right: -139.83
// top: 25.63, left: -77, right: -77
// ---

// 8: background: url(<path-to-image>) lightgray -168.667px -423px / 149.608% 224.412% no-repeat, #FFF;
// background: url(<path-to-image>) lightgray -76.892px -279.926px / 149.608% 224.412% no-repeat, #FFF;

// 9: background: url(<path-to-image>) lightgray 1357389797.985px -5925229194.061px / -2686965481.029% 4030447932.983% no-repeat, #FFF;
// background: url(<path-to-image>) lightgray 327908607.888px -4692894637.881px / -3532457089.514% 3650205285.055% no-repeat, #FFF;

// 10: top: -76.7, left: -66, right: -64.09
// top: 12.42, left: 68.85, right: 48.19
//
//
//
//
// MODALS:
//
//
// background: url(<path-to-image>) lightgray 0px -53.468px / 122.5% 183.75% no-repeat;
// background: url(<path-to-image>) lightgray 0px -55.484px / 100% 200% no-repeat, #FFF;

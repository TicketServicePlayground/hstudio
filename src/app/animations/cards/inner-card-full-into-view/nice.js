...
const MobileCard = ({ card, scrollYProgress, start, end }) => {
  const progress = useTransform(scrollYProgress, [start, end], [0, 1])

  const y = useTransform(progress, [0, 0.5], ['100vh', '0vh'])
  ...
  const metrics = React.useMemo(() => {
    ...
    if (isMobile) {
      cards.forEach((card, i) => {
        const height = Math.max(window.innerHeight, card.innerBlockHeight)
        sections.push({
          start: totalHeight / (window.innerHeight * cards.length),
          end: (totalHeight + height) / (window.innerHeight * cards.length),
        })
        totalHeight += height
      })
    } else {
